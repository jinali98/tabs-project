...
import Display from "./components/DisplayArea/Display";
import Pannel from "./components/SelectMenu/Pannel";
...
function App() {
  ...
  const menuItem = (id) => {
    const item = data.filter((each) => {
      return each.id === id;
    });

    setItem(item[0]);
    setDuties(item[0].duties);
  };
  ...
  return (
    <Container>
      ...
      <Pannel data={data} menuItem={menuItem} />
      <Display item={item} duties={duties} />
    </Container>
  );
}
